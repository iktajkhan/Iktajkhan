...
import { Calendar, Building, X, ExternalLink, Award, Sparkles, ChevronLeft, ChevronRight, ShieldCheck } from 'lucide-react';
...
export const ExperienceTimeline: React.FC = () => {
  ...
  return (
    <section id="experience" className="relative py-28 px-4 sm:px-6 max-w-7xl mx-auto overflow-hidden">
      ...
      {/* FEATURED SPOTLIGHT: Robi Axiata Chapter & Young Apex Summit 2026 */}
      <div className="mb-20">
        <div className="relative rounded-3xl p-1 bg-gradient-to-r from-[#da0037]/40 via-[#ff2a5f]/20 to-[#00f2fe]/40 shadow-2xl">
          <div className="glass-panel rounded-[22px] p-6 sm:p-10 border border-white/10 relative overflow-hidden bg-[#0a0a12]/90">
            {/* Top Badge Header */}
            <div className="flex flex-wrap items-center justify-between gap-3 mb-8">
              ...
              <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-[#da0037]/15 border border-[#da0037]/30 text-xs font-mono text-[#ff4d6d]">
                <ShieldCheck className="w-3.5 h-3.5" />
                <span>Robi Axiata Limited</span>
              </div>
            </div>

            {/* Dual Card Showcase for Robi: robiIktaj.png & robi.png */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              {/* Card 1: Official Executive Appointment Portrait */}
              <div className="lg:col-span-5 flex flex-col">
                <div
                  onClick={() => {
                    const idx = filteredMilestones.findIndex((m) => m.id === 'milestone-robi-role');
                    if (idx !== -1) setActiveModalIndex(idx);
                  }}
                  data-cursor="EXPAND"
                  className="group relative rounded-2xl overflow-hidden border border-white/15 bg-black/60 shadow-xl cursor-pointer aspect-[4/5] max-h-[420px]"
                >
                  <img
                    src={PERSONAL_INFO.images.robiPortrait}
                    alt="Md IkTaj Khan - Sales Apprentice / Territory Officer Robi Axiata Limited"
                  ...
                  <div className="absolute top-4 left-4">
                    <span className="px-3 py-1 rounded-full text-[11px] font-mono font-bold uppercase bg-[#da0037] text-white shadow-lg shadow-[#da0037]/50">
                      Current Appointment
                    </span>
                  </div>

                  {/* Caption on Image */}
                  <div className="absolute bottom-4 left-4 right-4 p-3.5 rounded-xl glass-panel border border-white/10 backdrop-blur-md">
              ...
              {/* Card 2: Red Velocity Young Apex Summit 2026 Stage Photo */}
              <div className="lg:col-span-7 flex flex-col justify-between h-full">
                <div>
                  <div className="flex items-center gap-2 text-xs font-mono text-white/50 mb-2">
                    <Calendar className="w-3.5 h-3.5 text-[#da0037]" />
                    <span>Summit 2026 • Fast &amp; Fearless</span>
                  </div>
                  <h3 className="font-heading font-black text-2xl sm:text-4xl text-white tracking-tight leading-snug">
                    Red Velocity Young Apex Summit 2026
                  </h3>
                  <p className="mt-3 text-sm sm:text-base text-white/80 font-sans leading-relaxed">
                    Celebrating collaborative velocity, fearless commercial execution, and nationwide distribution milestones alongside the executive leadership of <strong className="text-white">Robi Axiata Limited</strong> &amp; <strong className="text-white">Airtel</strong>.
                  </p>
                </div>
                ...
                <div
                  onClick={() => {
                    const idx = filteredMilestones.findIndex((m) => m.id === 'milestone-robi-summit');
                    if (idx !== -1) setActiveModalIndex(idx);
                  }}
                  data-cursor="EXPAND"
                  className="group relative mt-6 rounded-2xl overflow-hidden border border-white/15 bg-black/60 shadow-xl cursor-pointer aspect-video"
                >
                  <img
                    src={PERSONAL_INFO.images.robiSummit}
                    alt="Red Velocity Young Apex Summit 2026 - Robi Axiata and Airtel"
                  ...
                  <div className="absolute bottom-3 left-4 right-4 flex items-center justify-between">
                    <span className="text-xs font-mono text-white/90 font-medium">
                      Apex Stage • Red Velocity 2026
                    </span>
                    <span className="text-[11px] font-mono text-[#00f2fe] uppercase tracking-wider flex items-center gap-1.5">
                      <ExternalLink className="w-3 h-3" /> View Stage
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mt-6">
                  {['Territory Ops', 'Fast & Fearless', 'Apex Summit 2026', 'Airtel & Robi'].map((badge) => (
                    <div
                      key={badge}
                      className="px-3 py-2 rounded-xl glass-panel text-center text-[11px] font-mono font-medium text-white/80 border border-white/5"
                    >
                      {badge}
                    </div>
                  ))}
